import { CheckCircle2, XCircle, CircleHelp, ShieldCheck, Snowflake, Lock, Flame, Bug } from 'lucide-react'
...
function IconWrap({ children, title, color }: { children: React.ReactNode; title: string; color: string }) {
    return (
        <span title={title} aria-label={title} style={{ display: 'inline-flex', alignItems: 'center', color }}>
            {children}
        </span>
    )
}
...
export default function AuditIcons({ flags }: { flags: AuditFlags }) {
    ...
    return (
        <div className="audit-icons" style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}>
            ...
            <BoolIcon value={flags.locked} label="Liquidity Locked" trueIcon={<Lock size={size} />} falseIcon={<XCircle size={size} />} />
            <BoolIcon value={flags.burned} label="Burned" trueIcon={<Flame size={size} />} falseIcon={<XCircle size={size} />} />
            <BoolIcon value={flags.honeypot == null ? undefined : !flags.honeypot} label="Not Honeypot" trueIcon={<CheckCircle2 size={size} />} falseIcon={<Bug size={size} />} />
        </div>
    )
}
