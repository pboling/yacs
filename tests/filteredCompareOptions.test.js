...
test('returns empty when not open', () => {
    const res = computeFilteredCompareOptions({ open: false, allRows: rows, currentRow: rows[0], compareSearch: '' })
    assert.equal(res.length, 0)
})

test('excludes current row by id', () => {
    const res = computeFilteredCompareOptions({ open: true, allRows: rows, currentRow: rows[1], compareSearch: '' })
...
test('caps to top 100 when no search', () => {
    ...
    const res = computeFilteredCompareOptions({ open: true, allRows: many, currentRow: null, compareSearch: '' })
    assert.equal(res.length, 100)
})

test('filters by tokenName or tokenSymbol (case-insensitive)', () => {
    const res1 = computeFilteredCompareOptions({ open: true, allRows: rows, currentRow: null, compareSearch: 'beta' })
    assert.equal(res1.length, 1)
    assert.equal(res1[0].id, '2')

    const res2 = computeFilteredCompareOptions({ open: true, allRows: rows, currentRow: null, compareSearch: 'GAM' })
