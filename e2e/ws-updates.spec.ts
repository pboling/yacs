...
import { findTableForHeading, getCellTextByToken, parseCounter } from './helpers'
...
test('NewTokensSells', async ({ page }) => {
  ...
  await page
    ...
    .catch((err: unknown) => {
      ...
      console.log('DIAGNOSTIC: Tables load timeout. Error:', msg)
      throw err
    })
  ...
  const initialState = await tableEl.evaluate((table) => {
    ...
    const result = new Map<
      string,
      { buys: number; sells: number; maxCounter: number; rowId: string }
    >()
  ...
  console.log(
    `DIAGNOSTIC: [${tableName}] Sample parsed buys/sells (first 3):`,
    initialState.debugSamples.join(', '),
  )
  console.log(
    `DIAGNOSTIC: [${tableName}] Successfully read initial state for ${tokenStates.size} tokens`,
  )
  console.log(
    `DIAGNOSTIC: [${tableName}] Sample initial states (first 5):`,
    Array.from(tokenStates.entries())
      .slice(0, 5)
      .map(([token, state]) => `${token}: buys=${state.buys} sells=${state.sells}`)
      .join(', '),
  )
  ...
  console.log(
    `DIAGNOSTIC: [${tableName}] Starting to poll all ${tokenStates.size} tokens for updates...`,
  )
          ...
          async () => {
            try {
              ...
              if (currentMax > initial.maxCounter) {
                console.log(
                  `DIAGNOSTIC: [${tableName}] ✓ Token ${token} updated! Initial: buys=${initial.buys} sells=${initial.sells}, Current: buys=${buys} sells=${sells}`,
                )
              } else if (
                currentMax !== initial.maxCounter ||
                buys !== initial.buys ||
                sells !== initial.sells
              ) {
                console.log(
                  `DIAGNOSTIC: [${tableName}] Token ${token} changed: buys=${initial.buys}→${buys} sells=${initial.sells}→${sells}`,
                )
              }
  ...
  console.log(
    `DIAGNOSTIC: [${tableName}] Test passed! At least one token showed buys/sells increase.`,
  )
})
