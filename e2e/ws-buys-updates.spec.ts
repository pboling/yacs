...
test('Trending Tokens: Buys increases via WebSocket (same row key)', async ({ page }) => {
    ...
    await expect
        .poll(async () => parseBuys(await getCellTextByRowId(page, 'Trending Tokens', rowId, 8)), { timeout: 30_000, intervals: [100, 250, 500, 1000] })
...
test('New Tokens: Buys increases via WebSocket (same row key)', async ({ page }) => {
    ...
    await expect
        .poll(async () => parseBuys(await getCellTextByRowId(page, 'New Tokens', rowId, 8)), { timeout: 30_000, intervals: [100, 250, 500, 1000] })
        .toBeGreaterThan(baseline)
})
